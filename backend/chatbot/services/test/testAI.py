...
def test_explicit_max_tokens_is_forwarded_unchanged(monkeypatch):
    ...
    monkeypatch.setattr(ai_service, "get_openai_client", lambda: fake_client)
    ai_service.get_ai_reply([{"role": "user", "content": "Bonjour"}], max_tokens=100)
    _, kwargs = fake_client.chat.completions.create.call_args
    assert kwargs["max_tokens"] == 100
...
def test_multiple_matching_products_include_all_images():
    payload = json.loads(ai_service.format_response("Voici les produits.", "clavier"))
    assert "products" in payload
    assert len(payload["products"]) == 2
    assert "Clavier gamer" in {product["name"] for product in payload["products"]}
    assert all(product["image"].startswith("/api/chatbot/images/") for product in payload["products"])


def test_ordinateur_and_pc_return_two_products():
    payload = json.loads(
        ai_service.format_response("Voici les produits.", "ordinateur portable et PC gamer")
    )
    assert len(payload["products"]) == 2
    assert {product["image"] for product in payload["products"]} == {
        "/api/chatbot/images/ordinateure.png",
        "/api/chatbot/images/gamer.png",
    }
